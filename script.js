...
const accordion = triggerSelector => {
  const btns = document.querySelectorAll(triggerSelector); // blocks = document.querySelectorAll(itemsSelector);
...
const forms = () => {
  ...
  form.forEach(item => {
    item.addEventListener('submit', e => {
      ...
      console.log(api);
      Object(_services_requests__WEBPACK_IMPORTED_MODULE_1__["postData"])(api, formData).then(result => {
        console.log(result);
...
const modals = () => {
  ...
  function showModalByTime(selector, time) {
    setTimeout(function () {
      let display; // атрибут [data-modal] содержится во всех всплывающих окнах
...
const showMoreStyles = (trigger, wrapper) => {
  // wrapper - это обертка/блок куда мы будем помещать все
...
const sliders = (slides, dir, prev, next) => {
  ...
  showSlides(slideIndex); // ниже просто функция по перелистыванию
